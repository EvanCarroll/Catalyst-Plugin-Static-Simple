package Catalyst::Plugin::Static::Simple::ButMaintained;
...
sub _locate_static_file {
	...
	while ( @ipaths && --$count) {
		...
		if ( ref $dir eq 'CODE' ) {
			eval { $dpaths = &$dir( $c ) };
			if ($@) {
				$c->log->error( 'Static::Simple::ButMaintained: include_path error: ' . $@ );
			} else {
...
Catalyst::Plugin::Static::Simple::ButMaintained - Make serving static pages painless.

=head1 SYNOPSIS

	package MyApp;
	use Catalyst qw/ Static::Simple::ButMaintained /;
	MyApp->setup;
...
The Static::Simple::ButMaintained plugin is designed to make serving static content in
...
Though Static::Simple::ButMaintained is designed to work out-of-the-box, you can tweak
...
By default, Static::Simple::ButMaintained will deliver all files having extensions
...
configuration of Static::Simple::ButMaintained. Note also that files having any other
...
Static::Simple::ButMaintained extends the following steps in the Catalyst process.
